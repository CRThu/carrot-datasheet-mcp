...
**Carrot Datasheet MCP** 是一个专为硬件工程师打造的自动化文档处理流水线。它能够将复杂的硬件技术文档（`.docx`）转换为结构化、AI 可读的 Markdown 格式，并通过 MCP (Model Context Protocol) 服务直接集成到各类 IDE（如 Antigravity IDE, Claude Code 等）中，实现对 datasheet 的精准检索与深度解析。
